...
async function buildCore() {
  ...
  log.success(`Core styles built → ${outputPath}`);
}
...
`;
  ...
  log.verbose(`  CSS compiled → ${outputPath}`);
}
...
async function buildComponentJS(uswdsName, blockName, blockDir) {
  ...
  log.verbose(`  JavaScript created → ${outputPath}`);
}
...
`;
  ...
  log.verbose(`  README created → ${outputPath}`);
}
...
function parseStylelintOutput(output) {
  ...
  for (const line of lines) {
    ...
    if (line.match(/^[a-z]/i) && line.includes('.css')) {
      ...
      // Match error line (e.g., "   17:1    ✖  Expected empty line before comment")
      const match = line.match(/^\s+(\d+:\d+)\s+✖\s+(.+?)(?:\s{2,}(.+))?$/);
      if (match) {
        errors.push({
          file: currentFile,
          location: match[1],
